...
public class Lugar {
	...
	public Lugar(){}
	public Lugar(Player p , Inventory i)
	{	
		this.player=p;
		this.inventory=i;
	
	}
	public Lugar(int locationId,String name, String itemName,String obstacleName , Player p, Inventory i)
	{	
	...
	public boolean combat(int obsNum)
	{
		for(int i=0; i<obsNum;i++)
		{
			...
			playerStats();
			enemyStats();
			while(player.health>0 && obstacle.vida>0)
			{	
			...
			if(hit.equals("h"))
				...
				obstacle.vida=obstacle.vida-player.damage;
				System.out.println("Player Health: " + player.health);
				System.out.println("Enemy Health: " + obstacle.vida );
				System.out.println();
			...
			if(obstacle.vida<player.health)
			{
	...
	public void printMenu()
	{
		...
		if(select==1)
		{
			Lugar safehouse=new Safehouse(player,inventory);
			safehouse.getLocation();
		}
		else if(select==2){
			Lugar restaurant=new Restaurant(player,inventory);
			restaurant.getLocation();
		}
		
		else if(select==3){
			Lugar forest=new Forest(player,inventory);
			forest.getLocation();
		}
		
		else if(select==4){
			Lugar river=new River(player,inventory);
			river.getLocation();
		}
		
		else if(select==5){
			Lugar toolstore=new Toolstore(player,inventory);
			toolstore.getLocation();
		}
		else {
			
			Lugar helpcenter=new Help(player,inventory);
